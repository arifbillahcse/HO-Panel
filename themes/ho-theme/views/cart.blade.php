            ...
            <div class="flex flex-row justify-between w-full bg-background-secondary p-3 rounded-md border border-neutral">
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-sm">
                        ...
                        @endforeach
                        {{-- Checkout fields the server module asked for, such as
                             the domain name. The default theme collects these but
                             never shows them back, so a cart of three domains reads
                             as three identical ".com" lines. --}}
                        @foreach ($item->checkout_config ?? [] as $key => $value)
                        @if (filled($value) && !is_array($value))
                        <span class="font-medium text-base">{{ $value }}</span><br>
                        @endif
                        @endforeach
                    </p>
                </div>
