...
<div class="container mt-14">
    ...
    <a href="{{ route('cosmotown.domains') }}" wire:navigate
        class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:text-primary/80">
        <x-ri-arrow-right-fill class="size-4 rotate-180" />
        All domains
    </a>

    <h1 class="text-3xl font-bold mt-3 break-all">{{ $name }}</h1>
    ...
    @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8 items-start">
            ...
            {{-- Nameservers: the one thing the service page cannot already do. --}}
            <div class="card p-6 lg:col-span-2">
                <h2 class="text-xl font-semibold">Nameservers</h2>
                <p class="text-sm text-base/60 mt-1.5">
                    These decide which servers answer for your domain. Enter at least two.
                </p>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="card p-6">
                    ...
                    <dl class="mt-4 flex flex-col gap-3 text-sm">
                        <div class="flex justify-between gap-4">
                            <dt class="text-base/60">Expires</dt>
                            <dd class="font-medium text-right">{{ $details['expiration_date'] ?? 'unknown' }}</dd>
                        </div>
                        <div class="flex justify-between gap-4">
                        ...
                        <div class="flex justify-between gap-4">
                            <dt class="text-base/60">Price</dt>
                            <dd class="font-medium text-right">{{ $service->formattedPrice }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="card p-6">
                    ...
                    <p class="text-sm mt-1 {{ $locked ? 'text-success' : 'text-base/50' }}">
                    ...
                    <p class="text-sm mt-1 {{ $privacy ? 'text-success' : 'text-base/50' }}">
                        {{ $privacy ? 'On' : 'Off' }}
                    </p>
                </div>
